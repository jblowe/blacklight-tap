...
errorfile = open(f'{DTYPE}_errors.csv', 'w')
...
with open(output_file, 'w') as outputfile:
    ...
    header = 'Season Roll# Count Missing Extra'.split(' ')
        ...
        for counter, roll in enumerate(zz):
            ...
            for e in roll_response.results:
                ...
                try:
                    ...
                    if 'EXP_s' in e:
                        write_errors('exposure is not a number', [e['EXP_s']])
                    else:
            ...
            # skip empty rolls: they are likely apocryphal
            if len(exposure_list) == 0:
                write_errors('roll with no exposures', [roll])
                continue
            csvoutput.writerow([season, roll, len(exposure_list),
